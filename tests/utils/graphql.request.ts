...
export type GetTodosQuery = (
  ...
  & { todo: Array<(
    { __typename?: 'todo' }
    & Pick<Todo, 'id' | 'description' | 'is_done'>
  )> }
);
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    ...
    AddTodo(variables?: AddTodoMutationVariables, requestHeaders?: Dom.RequestInit["headers"]): Promise<AddTodoMutation> {
      return withWrapper((wrappedRequestHeaders) => client.request<AddTodoMutation>(AddTodoDocument, variables, {...requestHeaders, ...wrappedRequestHeaders}), 'AddTodo');
    }
  };
}
